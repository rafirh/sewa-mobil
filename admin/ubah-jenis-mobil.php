          ...
          <div class="card">
            <div class="card-body">
              <div class="mb-3 row">
                ...
                <div class="col">
                  <input required type="text" class="form-control" placeholder="Masukkan nama jenis mobil" name="nama" value="<?= $mobil['nama'] ?>">
                </div>
              </div>
            </div>
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  if (checkRequiredFields(['nama'])) {
    ...
    $query = "UPDATE jenis_mobil SET nama = '$nama' WHERE id = $id";
    $result = mysqli_query($conn, $query);

    if ($result) {
      setFlashMessage('success', 'Jenis mobil berhasil diubah!');
      redirectJs('jenis-mobil.php');
      exit;
    } else {
      setFlashMessage('error', 'Jenis mobil gagal diubah!');
      redirectJs("ubah-jenis-mobil.php?id=$id");
      exit;
    }
  } else {
