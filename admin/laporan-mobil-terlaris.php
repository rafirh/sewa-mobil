<?php
$active = 'laporan-mobil-terlaris';
$title = 'Laporan Mobil Terlaris';
...
$query = "
  ...
  LEFT JOIN agen ON agen.id = mobil.agen_id
  LEFT JOIN merk_mobil ON merk_mobil.id = mobil.merk_id
...
$merk = getAll($conn, 'merk_mobil');
$cc = getAll($conn, 'cc');
$transmisi = getAll($conn, 'transmisi');
?>
  ...
  <div class="container-xl">
    ...
    <div class="row g-2 align-items-center mt-2">
      <div class="col d-flex">
        ...
        <?php if (isParamsExist(['start_date', 'end_date', 'agen_id', 'merk_id'])) : ?>
          <a href="laporan-mobil-terlaris.php" class="btn btn-outline-danger" data-bs-toggle="tooltip" data-bs-original-title="Clear filter" data-bs-placement="bottom">
              ...
              <tbody>
                <?php foreach ($laporan as $index => $item) : ?>
                  <tr class="text-muted">
                    ...
                    <td>
                      <?= $item['nama_mobil'] ?> (<?= $item['merk_mobil'] ?>)
                    </td>
                    <td>
                      <?= $item['tahun_mobil'] ?>
                    </td>
            ...
            <div class="col-12 mb-3">
              ...
              <select class="form-select" name="merk_id">
                ...
                <?php foreach ($merk as $item) : ?>
                  <option value="<?= $item['id'] ?>" <?= isset($_GET['merk_id']) && $_GET['merk_id'] == $item['id'] ? 'selected' : '' ?>>
                    <?= $item['nama'] ?>
                  </option>
                <?php endforeach ?>
              </select>
