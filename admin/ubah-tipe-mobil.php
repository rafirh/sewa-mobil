          ...
          <div class="card">
            <div class="card-body">
              <div class="mb-3 row">
                ...
                <div class="col">
                  <input required type="text" class="form-control" placeholder="Masukkan nama tipe mobil" name="nama" value="<?= $mobil['nama'] ?>">
                </div>
              </div>
            </div>
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  if (checkRequiredFields(['nama'])) {
    ...
    $query = "UPDATE tipe_mobil SET nama = '$nama' WHERE id = $id";
    $result = mysqli_query($conn, $query);

    if ($result) {
      setFlashMessage('success', 'Tipe mobil berhasil diubah!');
      redirectJs('tipe-mobil.php');
      exit;
    } else {
      setFlashMessage('error', 'Tipe mobil gagal diubah!');
      redirectJs("ubah-tipe-mobil.php?id=$id");
      exit;
    }
  } else {
