...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  if (checkRequiredFields(['merk_id', 'jenis_id', 'transmisi_id', 'warna_id', 'nama', 'plat_nomor', 'tahun', 'harga', 'kapasitas', 'cc_id'])) {
    ...
    $agen_id = htmlspecialchars($_POST['agen_id']);
    ...
    $query = "UPDATE mobil SET merk_id = '$merk_id', jenis_id = '$jenis_id', transmisi_id = '$transmisi_id', warna_id = '$warna_id', cc_id = '$cc_id', tipe_id = '$tipe_id', nama = '$nama', plat_nomor = '$plat_nomor', tahun = '$tahun', harga = '$harga', kapasitas = '$kapasitas', foto = '$foto', agen_id = '$agen_id' WHERE id = $id";
      ...
      <div class="col-md-10">
        <form action="ubah-mobil.php?id=<?= $mobil['id'] ?>" method="POST" enctype="multipart/form-data">
          <div class="card">
            <div class="card-body">
              <div class="mb-3 row">
                <label class="col-md-4 col-12 col-form-label required">Agen</label>
                <div class="col">
                  <select required class="form-select" name="agen_id">
                    <option value="" selected disabled>Pilih agen</option>
                    <?php foreach ($agen as $item) : ?>
                      <option value="<?= $item['id'] ?>" <?= $item['id'] == $mobil['agen_id'] ? 'selected' : '' ?>>
                        <?= $item['nama'] ?>
                      </option>
                    <?php endforeach ?>
                  </select>
