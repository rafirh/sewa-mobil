<?php
$active = 'laporan-pelanggan-teraktif';
$title = 'Laporan Pelanggan Teraktif';
...
$query = "
  SELECT
    ...
    SUM(transaksi.total_harga) AS total_pengeluaran
  FROM transaksi
  RIGHT JOIN user ON user.id = transaksi.user_id
...
$query .= "
  GROUP BY user.id
  ORDER BY total_transaksi DESC, total_pengeluaran DESC
";
    ...
    <div class="row g-2 align-items-center mt-2">
      <div class="col d-flex">
        ...
        <?php if (isParamsExist(['start_date', 'end_date'])) : ?>
          <a href="laporan-pelanggan-teraktif.php" class="btn btn-outline-danger" data-bs-toggle="tooltip" data-bs-original-title="Clear filter" data-bs-placement="bottom">
              ...
              <tbody>
                <?php foreach ($laporan as $index => $item) : ?>
                  <tr class="text-muted">
                    ...
                    <td>
                      <?= format_rupiah($item['total_pengeluaran']) ?>
                    </td>
                  </tr>
                <?php endforeach ?>
