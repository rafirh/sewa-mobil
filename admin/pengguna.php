...
if (isParamsExist(['q'])) {
  ...
  $query .= " AND (nama LIKE '%$q%' OR email LIKE '%$q%' OR alamat LIKE '%$q%' OR no_hp LIKE '%$q%' OR role LIKE '%$q%')";
}

if (isParamsExist(['role'])) {
...
<div class="page-header d-print-none mt-2">
  <div class="container-xl">
    <div class="row g-2 align-items-center">
      ...
      <div class="col-auto ms-auto d-print-none mt-3">
        <div class="btn-list d-flex">
          <a href="tambah-pengguna.php" class="btn btn-primary d-none d-sm-inline-block" id="btnAdd">
          ...
          <a href="tambah-pengguna.php" class="btn btn-primary d-sm-none btn-icon" data-bs-tooltip="Tambah Pengguna" data-bs-placement="left">
            <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-plus" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
              <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
              <path d="M12 5l0 14"></path>
              <path d="M5 12l14 0"></path>
    ...
    <div class="row g-2 align-items-center">
      ...
      <?php if (isParamsExist(['q', 'sortby', 'order', 'role'])) : ?>
        <div class="col-auto mt-3">
          <a href="pengguna.php" class="btn btn-outline-danger btn-icon" data-bs-toggle="tooltip" data-bs-original-title="Clear filter" data-bs-placement="bottom">
        ...
        <div class="card">
          <div class="table-responsive">
            <table class="table card-table table-vcenter text-nowrap datatable">
              ...
              <tbody>
                <?php foreach ($merk as $index => $item) : ?>
                  <tr class="text-muted">
                    <td>
                      <?= $index + 1 ?>
                    </td>
                    <td>
                      <a class="avatar me-2 image-preview cursor-pointer" data-fslightbox="image-<?= $index + 1 ?>" href="<?= asset($item['foto'] != '' ? $item['foto'] : 'images/default.png') ?>" style="background-image: url(<?= asset($item['foto'] != '' ? $item['foto'] : 'images/default.png') ?>)">
                      </a>
                    </td>
                    <td>
                    ...
                    <td>
                      <span <?= add_title_tooltip($item['email'], 24) ?>>
                        <?= mb_strimwidth($item['email'], 0, 24, '...') ?>
                      </span>
                    </td>
                    <td>
                      <span <?= add_title_tooltip($item['alamat'], 24) ?>>
                        <?= mb_strimwidth($item['alamat'], 0, 24, '...') ?>
                      </span>
                    </td>
                    <td>
                      <span <?= add_title_tooltip($item['no_hp'], 24) ?>>
                        <?= mb_strimwidth($item['no_hp'], 0, 24, '...') ?>
                      </span>
                    </td>
                    <td>
                      ...
                      <?php elseif ($item['role'] == 'agent'): ?>
                        <span class="badge bg-teal-lt">
                          <?= ucfirst($item['role']) ?>
                        </span>
                      <?php elseif ($item['role'] == 'customer'): ?>
                    ...
                    <td>
                      <div class="d-flex justify-content-center">
                        ...
                        <div class="text-muted dropdown-menu dropdown-menu-end">
                          <a class="dropdown-item" href="ubah-pengguna.php?id=<?= $item['id'] ?>">
          ...
          <div class="row">
            <div class="col-12 mb-3">
              ...
              <select class="form-select" name="role">
                ...
                <option value="administrator" <?= ($_GET['role'] ?? '') == 'administrator' ? 'selected' : '' ?>>
                  Administrator
                </option>
                <option value="agent" <?= ($_GET['role'] ?? '') == 'agent' ? 'selected' : '' ?>>
                  Agen
                </option>
                <option value="customer" <?= ($_GET['role'] ?? '') == 'customer' ? 'selected' : '' ?>>
                  Pelanggan
                </option>
              </select>
            </div>
      ...
      <div class="modal-footer">
        <div class="w-100">
          <div class="row">
            ...
            <div class="col">
              <form method="post" id="formDelete" action="hapus-pengguna.php">
